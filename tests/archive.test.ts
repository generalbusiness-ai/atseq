...
test('retain complete app history and rebuild without the PDS', async t => {
  ...
  try {
    ...
    await check('small imported data produces a zero-based chart and static source-tagged exports',async()=>{
      ...
      const source=await SourceBundle.pack(fixture.manifest,{...fixture.files,'import.csv':new TextEncoder().encode(csv),'schemas/summary.json':new TextEncoder().encode(JSON.stringify(doc)),'summary.jsonata':new TextEncoder().encode('{"count":$count(state.readings),"total":($count(state.readings)=0?0:$sum(state.readings.millimetres)),"rows":state.readings}')});
  ...
  } finally { await recordFlowEvidence('archive',results,{expectedCases:8,browserVersion:browser.version(),offlineServiceWorker:true});await browser.close();await service.close();await env.close();await resetDisposable(env.dir); }
});
