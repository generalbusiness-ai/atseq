...
assert.equal(transcript.steps.length,26);assert.ok(transcript.steps.every((step:any)=>step.command==='npm run --silent atseq'&&step.exitCode===0&&step.response.ok));
...
await writeFile(root+'/verification.json',JSON.stringify({passed:true,measuredAt:new Date().toISOString(),title:result.invitation.title,authoring:'Existing agent, documented adapter only; separate from fixture generation and final independent review.',hostAndBuildUnchanged:true,hostFilesCompared:Object.keys(before.hashes).length,adapterCalls:26,entries:5,source:source.root,genesis:replay.anchor.cid,frontier:replay.snapshot.projection.frontier,sourceHashes},null,2)+'\n');console.log('Agent-authored app: source and 5 signed entries replay; 26 adapter calls, same exact retry, unchanged host/build.');
